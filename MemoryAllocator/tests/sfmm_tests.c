...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	...
	void *y = sf_memalign(sizeof(int), 64);
	
	long alignPtr = (long) y;
	cr_assert_not_null(x, "x is NULL!");
	cr_assert_not_null(y, "y is NULL!");
	cr_assert(alignPtr % 64 == 0, "y is not aligned!");
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
	...
	void *z = sf_memalign(sizeof(int), 64);
	sf_show_heap();
	long alignPtr = (long) z;
	cr_assert_not_null(x, "x is NULL!");
	cr_assert_not_null(y, "y is NULL!");
	cr_assert_not_null(z, "z is NULL!");
	cr_assert(alignPtr % 64 == 0, "z is not ALIGNED!");
...
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
	...
	void *x = sf_memalign(sizeof(int), 50);
